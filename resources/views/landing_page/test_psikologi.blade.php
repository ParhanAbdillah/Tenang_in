    ...
    <section class="relative min-h-[700px] flex items-center pt-32 pb-20 overflow-hidden bg-white nav-font">
        <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div class="z-10 space-y-8">
                <div
                    class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-orange-50 border border-orange-100 text-[#D98324] font-bold text-sm tracking-wide">
                ...
                <p class="text-gray-500 text-lg md:text-xl max-w-lg leading-relaxed">
                    Yuk, kenali kondisimu dengan mengikuti Tes Psikologis dari <span
                        class="font-bold text-[#0A4D68]">Tenang-in.</span> Temukan jawaban untuk melangkah lebih yakin
                    dan tenang.
                </p>
                <div class="flex flex-wrap gap-4">
                    <a href="#daftar-tes"
                        class="bg-[#D98324] text-white px-10 py-4 rounded-2xl font-bold text-lg hover:bg-[#b86d1d] transition-all transform hover:-translate-y-1 shadow-xl shadow-orange-200/50">
                        Coba Tes Sekarang
                    </a>
                </div>
            </div>
            ...
            <div class="relative grid grid-cols-3 gap-4 h-[550px]">
                <div class="flex flex-col gap-4 mt-20">
                    <div class="h-56 rounded-[2.5rem] overflow-hidden shadow-2xl rotate-[-2deg] border-4 border-white">
                        <img src="https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?auto=format&fit=crop&q=80"
                            class="w-full h-full object-cover">
                    </div>
                </div>
                <div class="flex flex-col gap-4">
                    <div class="h-44 rounded-[2.5rem] overflow-hidden shadow-2xl border-4 border-white">
                        <img src="https://images.unsplash.com/photo-1516589174184-c68526614488?auto=format&fit=crop&q=80"
                            class="w-full h-full object-cover">
                    </div>
                    <div class="h-64 rounded-[2.5rem] overflow-hidden shadow-2xl border-4 border-white">
                        <img src="https://images.unsplash.com/photo-1529333166437-7750a6dd5a70?auto=format&fit=crop&q=80"
                            class="w-full h-full object-cover">
                    </div>
                </div>
                <div class="flex flex-col gap-4 mt-12">
                    <div class="h-60 rounded-[2.5rem] overflow-hidden shadow-2xl rotate-[2deg] border-4 border-white">
                        <img src="https://images.unsplash.com/photo-1499209974431-9dac3adaf471?auto=format&fit=crop&q=80"
                            class="w-full h-full object-cover">
                    </div>
        ...
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                @foreach ($daftar_tes as $tes)
                    <div
                        class="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100 flex gap-6 hover:shadow-md transition-shadow">

                        <div class="w-32 h-32 flex-shrink-0 rounded-2xl overflow-hidden shadow-inner">
                            @if ($tes->image)
                                {{-- Jika ada gambar yang diupload di admin --}}
                                <img src="{{ asset('storage/' . $tes->image) }}" class="w-full h-full object-cover"
                                    alt="{{ $tes->name }}">
                            @else
                                {{-- Fallback ke gambar default jika admin tidak upload --}}
                                <img src="https://images.unsplash.com/photo-1513258496099-48168024adb0?auto=format&fit=crop&q=80"
                                    class="w-full h-full object-cover" alt="{{ $tes->name }}">
                            @endif
                        </div>
                        ...
                        <div class="flex flex-col justify-between flex-grow">
                            ...
                            <div class="flex justify-end mt-4">
                                {{-- Gunakan theme_color dari database untuk tombol jika ada --}}
                                <a href="{{ route('tes.detail', $tes->id) }}"
                                    class="text-white px-6 py-2 rounded-xl font-bold text-sm transition-colors shadow-lg shadow-orange-200"
                                    style="background-color: {{ $tes->theme_color ?? '#D98324' }}">
                                    Mulai Tes
                                </a>
                            </div>
                        </div>
